...
SELECT DEPT.DEPTNO FROM DEPT LEFT JOIN EMP ON EMP.DEPTNO = DEPT.DEPTNO WHERE ENAME IS NULL;
/*List the No.of emp’s and Avg salary within each department for each job. */
SELECT COUNT(EMP.ENAME), AVG(EMP.SAL) FROM EMP,DEPT WHERE EMP.DEPTNO=DEPT.DEPTNO GROUP BY JOB;
...
SELECT DEPT.* FROM EMP,DEPT WHERE EMP.DEPTNO = DEPT.DEPTNO GROUP BY DEPT.DNAME HAVING (COUNT(EMP.ENAME) > 2);
/* List the no. of emps in each department where the no. is more than 3.*/
SELECT EMP.ENAME FROM EMP GROUP BY EMP.DEPTNO HAVING COUNT(EMP.ENAME)>3;
...
/*List the Deptno and their average salaries for dept with the average salary less than the averages for all departments.*/ 
